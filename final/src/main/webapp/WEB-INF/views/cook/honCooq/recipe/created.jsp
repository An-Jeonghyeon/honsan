...
<script type="text/javascript">
	function check() {
		...
		f.action = "${pageContext.request.contextPath}/cook/honCooq/recipe/${mode}";
...
<article class="recipe_writeHeader">
	<div class="recipe_writeheaderInnerText">
		<h3 class="recipe_FadeinText">HOONCOOQ > recipe</h3>
		<br> <small class="recipe_FadeinText"> 혼자서 재료 준비는 물론 요리에
			주방 정리까지 하느라 바쁜 당신! <br> 그래도 스스로를 위해 요리를 하다 보면 점점 더 쉽고 편한 나만의 팁들을
			깨닫고는 하죠. <br> 오늘도 고군분투하고 있는 혼족들을 위해 나만의 요리팁이나 주방팁을 공유해 보는 건
			어떤가요?
		</small>
	</div>
	<div class="recipe_writeHeaderMenu">
		<ul>
			<li><a
				href="${pageContext.request.contextPath}/cook/honCook/recipe/list">전체글보기</a></li>
			<li><a
				href="${pageContext.request.contextPath}/cook/honCook/recipe/list">베스트글</a></li>
		</ul>
	</div>
</article>
<form method="post" name="writeBodyForm"
	onsubmit="return submitContents(this)" enctype="multipart/form-data">
	<article class="recipe_writeBody">
		<div class="recipe_writeBodyHeader">
			<span class="recipe_writeBodyHeader-Title">recipe</span>
		</div>
		<div class="recipe_subjectTitle">
			<span class="recipe_smallTitle">제목</span>
		</div>
		<div class="recipe_subjectBody">
			<input type="text" name="subject" id="subject"
				placeholder="제목을 입력해 주세요." value="${dto.subject}">
		</div>

		<div class="recipe_subjectTitle">
			<span class="recipe_smallTitle">분류선택</span>
		</div>
		<div class="">
			<select name="category" class="recipe_select" id="category">
		...
		<div class="recipe_contentBody">
			<div class="recipe_editor-area">
				<ul class="recipe_notice-list">
			...
			<div class="recipe_inputBody1">
				<textarea name="content" id="content">${dto.content}</textarea>
			</div>
		</div>

		<div class="recipe_subjectTitle">
			<span class="recipe_smallTitle">유튜브 URL</span>
		</div>
		<div class="recipe_subjectBody">
		...
		<div class="recipe_subjectTitle">
			<span class="recipe_smallTitle">유튜브 영상 자막</span>
		</div>
		<div class="recipe_subjectBody">
		...
		<div class="recipe_buttonBoxBody">
			<button type="submit" id="recipe_writeSubmit" class="recipe_btn">${mode=='update'?'수정완료':'등록하기'}</button>
			<button type="button" id="recipe_CancelSubmit" class="recipe_btn" onclick="javascript:location.href='${pageContext.request.contextPath}/cook/honCooq/recipe/list';">${mode=='update'?'수정취소':'등록취소'}</button>
		</div>
