...
//		sb.append("</style>");
		...
		n=current_page-numPerBlock;
		if(total_page > numPerBlock && currentPageSetup > 0) {
			sb.append("<a href='"+list_url+"page=1'><div class='cookTip_list-paging'>처음</div></a>");
			sb.append("<a href='"+list_url+"page="+n+"'><div class='cookTip_list-paging'>&lt;</div></a>");
		}
		...
		if(total_page-currentPageSetup>numPerBlock) {
			sb.append("<a href='"+list_url+"page="+n+"'><div class='cookTip_list-paging'>&gt;</div></a>");
			sb.append("<a href='"+list_url+"page="+total_page+"'><div class='cookTip_list-paging'>끝</div></a>");
		}
		sb.append("</div>");
	 ...
	 * @return							페이징 처리 결과
	 */
	public String paging3(int current_page, int total_page, String list_url) {
		StringBuffer sb=new StringBuffer();
		...
		sb.append("<style type='text/css'>");
		sb.append("#interior_paginate {clear:both;font:12px \"맑은 고딕\",NanumGothic,돋움,Dotum,AppleGothic;padding:15px 0px 0px 0px;text-align:center;height:28px;white-space:nowrap;}");
		sb.append("</style>");
		
		sb.append("<div id='interior_paginate'>");
		// 처음페이지, 이전(10페이지 전)
		n=current_page-numPerBlock;
		if(total_page > numPerBlock && currentPageSetup > 0) {
		...
		while(page<=total_page && page <=(currentPageSetup+numPerBlock)) {
			if(page==current_page) {
				sb.append("<span class='curBox' id='curBox_sub'>"+page+"</span>");
			} else {
				sb.append("<a href='"+list_url+"page="+page+"' class='numBox' id='numBox_sub'>"+page+"</a>");
			}
			page++;
		}
	 ...
	 * @return							페이징 처리 결과
	 */
    public String pagingMethod3(int current_page, int total_page, String methodName) {
		if(current_page < 1 || total_page < 1) {
			return "";
		}
        ...
        n = current_page - numPerBlock;
        if ((total_page > numPerBlock) && (currentPageSetUp > 0)) {
			//sb.append("<a id='tlBox' onclick='"+methodName+"(1);'>처음</a>");
			sb.append("<a id='tlBox' onclick='"+methodName+"("+n+");'><i class='fas fa-angle-left'></i></a>");
        }
        ...
        while((page <= total_page) && (page <= currentPageSetUp + numPerBlock)) {
           if(page == current_page) {
			   ...
			   sb.append("<a onclick='"+methodName+"("+page+");' class='numBox' id='numBox_sub'>"+page+"</a>");
           }
           page++;
        }
		...
		if(n>total_page) n=total_page;
        if (total_page - currentPageSetUp > numPerBlock) {
			sb.append("<a id='trBox' onclick='"+methodName+"("+n+");'><i class='fas fa-angle-right'></i></a>");
			//sb.append("<a id='tlBox' onclick='"+methodName+"("+total_page+");'>끝</a>");
        }
		sb.append("</div>");
